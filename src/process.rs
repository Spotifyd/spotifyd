...
fn audio_item_to_env(audio_item: Box<AudioItem>, env: &mut HashMap<&str, String>) {
    env.insert("TRACK_ID", audio_item.track_id.to_id());
...
pub(crate) fn spawn_program_on_event(
    ...
    match event {
        ...
        PlayerEvent::Playing {
            ...
            env.insert("TRACK_ID", track_id.to_id());
        ...
        PlayerEvent::SetQueue { context_uri, .. } => {
            env.insert("PLAYER_EVENT", "setqueue".to_string());
            env.insert("CONTEXT_URI", context_uri);
        }
    }
